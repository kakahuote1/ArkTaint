...
function buildModifiers(node) {
    ...
    if (ohos_typescript_1.default.canHaveModifiers(node)) {
        ohos_typescript_1.default.getModifiers(node)?.forEach(modifier => {
    ...
    if (ohos_typescript_1.default.isQualifiedName(exprNameNode)) {
        if (exprNameNode.left.getText(sourceFile) === TSConst_1.THIS_NAME) {
            ...
            if (arkInstance instanceof ArkMethod_1.ArkMethod) {
                ...
                const baseLocal = arkInstance.getBody()?.getLocals().get(TSConst_1.THIS_NAME) ??
                    new Local_1.Local(TSConst_1.THIS_NAME, new Type_1.ClassType(arkInstance.getDeclaringArkClass().getSignature(), arkInstance.getDeclaringArkClass().getGenericsTypes()));
                opValue = new Ref_1.ArkInstanceFieldRef(baseLocal, fieldSignature);
            }
            else if (arkInstance instanceof ArkClass_1.ArkClass) {
                const fieldSignature = arkInstance.getFieldWithName(fieldName)?.getSignature() ?? ArkSignatureBuilder_1.ArkSignatureBuilder.buildFieldSignatureFromFieldName(fieldName);
