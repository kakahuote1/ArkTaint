...
class Local {
    ...
    inferType(arkMethod) {
        if (this.name === TSConst_1.THIS_NAME && this.type instanceof Type_1.UnknownType) {
            ...
            const type = TypeInference_1.TypeInference.inferBaseType(this.name, arkMethod.getDeclaringArkClass()) ??
                ModelUtils_1.ModelUtils.findDeclaredLocal(this, arkMethod)?.getType();
            if (type) {
                this.type = type;
            }
        }
    ...
    getSignature() {
        return (this.signature ??
            new ArkSignature_1.LocalSignature(this.name, new ArkSignature_1.MethodSignature(ArkSignature_1.ClassSignature.DEFAULT, ArkSignatureBuilder_1.ArkSignatureBuilder.buildMethodSubSignatureFromMethodName(Const_1.UNKNOWN_METHOD_NAME))));
    }
