...
function parseObjectLiteral(objectLiteralCls, scene) {
    let map = new Map();
    if (objectLiteralCls?.getCategory() !== ArkClass_1.ClassCategory.OBJECT) {
        return map;
    }
...
class ViewTreeImpl extends TreeNodeStack {
    ...
    viewComponentCreationParser(name, stmt, expr) {
        ...
        if (builder) {
            ...
            if (!method?.hasBuilderDecorator()) {
                method?.addDecorator(new Decorator_1.Decorator(EtsConst_1.BUILDER_DECORATOR));
            }
    ...
    parseInstanceInvokeExpr(local2Node, stmt, expr, shouldPush = true) {
        ...
        if (local2Node.has(temp)) {
            ...
            if (component?.name === EtsConst_1.COMPONENT_REPEAT && expr.getMethodSignature().getMethodSubSignature().getMethodName() === 'each') {
                let arg = expr.getArg(0);
                ...
                component?.addStmt(local2Node, this, stmt);
            }
            return component;
        }
...
//# sourceMappingURL=ViewTreeBuilder.js.map