...
export { fromContainerFieldKey, toContainerFieldKey } from "../../core/kernel/contracts/SemanticPack";
...
export function collectContainerSlotLoadNodeIds(objId: number, slot: string, pag: Pag, scene: Scene): number[] {
    ...
    for (const rawNode of pag.getNodesIter()) {
        ...
        collectContainerSlotLoadNodeIdsForBaseLocal(val, slot, pag, scene, results, dedup);
    }

    return results;
}

export function collectContainerSlotLoadNodeIdsFromLocal(base: Local, slot: string, pag: Pag, scene: Scene): number[] {
    const results: number[] = [];
    const dedup = new Set<number>();
    collectContainerSlotLoadNodeIdsForBaseLocal(base, slot, pag, scene, results, dedup);
    return results;
}

export function collectArrayForEachCallbackParamNodeIdsFromTaintedLocal(local: Local, pag: Pag, scene: Scene): number[] {
    const results: number[] = [];
    ...
    for (const info of collectContainerSlotStoresFromTaintedLocal(local, pag)) {
        ...
        const callbackNodeIds = collectArrayCallbackParamNodeIds(info.objId, pag, scene, "forEach", [0]);
...
export function collectArrayHigherOrderEffectsFromTaintedLocal(local: Local, pag: Pag, scene: Scene): ArrayHigherOrderEffects {
    ...
    for (const info of collectContainerSlotStoresFromTaintedLocal(local, pag)) {
        ...
        const effect = collectArrayHigherOrderEffectsForObj(info.objId, info.slot, pag, scene);
...
export function collectArrayFromMapperCallbackParamNodeIdsForObj(
    ...
    for (const rawNode of pag.getNodesIter()) {
        ...
        for (const nodeId of collectArrayFromMapperCallbackParamNodeIdsFromTaintedLocal(val, pag, scene)) {
            if (dedup.has(nodeId)) continue;
            dedup.add(nodeId);
            results.push(nodeId);
        }
...
function isContainerMutationForBase(
    methodName: string,
    containerKind: "array" | "map" | "weakmap" | "set" | "weakset" | "list" | "queue" | undefined
): boolean {
    ...
    if (containerKind === "queue") return methodName === "add" || methodName === "append" || methodName === "push";
    return false;
}
...
function resolveContainerKind(base: Local, sig: string): "array" | "map" | "weakmap" | "set" | "weakset" | "list" | "queue" | undefined {
    ...
    if (text.includes("Map<") || text === "Map") return "map";
    if (text.includes("WeakMap<") || text === "WeakMap") return "weakmap";
    if (text.includes("Set<") || text === "Set") return "set";
    if (text.includes("WeakSet<") || text === "WeakSet") return "weakset";
    if (text.includes("List<") || text.endsWith("List")) return "list";
    if (text.includes("Queue<") || text.endsWith("Queue")) return "queue";
...
function isMapLikeStoreMethod(methodName: string): boolean {
    return methodName === "set" || methodName === "put" || methodName === "setObject";
}

function isMapLikeLoadMethod(methodName: string): boolean {
    return methodName === "get" || methodName === "getObject";
}
...
function collectArrayCallbackParamNodeIds(
    ...
    for (const rawNode of pag.getNodesIter()) {
        ...
        for (const stmt of val.getUsedStmts()) {
            ...
            const callbackNodeIds = collectCallbackParamNodeIds(scene, pag, args[0], paramIndexes);
            for (const nodeId of callbackNodeIds) {
                if (dedup.has(nodeId)) continue;
                dedup.add(nodeId);
                results.push(nodeId);
...
function collectContainerSlotLoadNodeIdsForBaseLocal(
    ...
    for (const stmt of val.getUsedStmts()) {
        ...
        if (stmt instanceof ArkInvokeStmt) {
            ...
            const callbackParamNodeIds = collectCallbackParamNodeIds(scene, pag, args[0], callbackParamIndexes);
