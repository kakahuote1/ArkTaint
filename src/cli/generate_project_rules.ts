...
async function collectSourceCandidates(
    ...
    const arkMainMethods = await collectArkMainMethods(scene, options, modelInvoker);
...
async function collectCandidatesFromScene(
    ...
    const sourceCandidates = await collectSourceCandidates(scene, sourceDir, options, modelInvoker);
...
export async function generateProjectRuleScaffold(options: GenerateProjectRuleCliOptions): Promise<RuleScaffoldResult> {
    ...
    const modelInvoker = createExternalEntryModelInvokerFromEnv({
