...
function countKinds(asset: AssetDocumentBase): Record<RuleBundleKind, number> {
    ...
    for (const binding of asset.bindings || []) {
        if (binding.role === "source")
            counts.sources++;
        if (binding.role === "sink")
            counts.sinks++;
        if (binding.role === "sanitizer")
            counts.sanitizers++;
        if (binding.role === "transfer")
            counts.transfers++;
    }
    return counts;
}
...
function verifyKindFirstRoot(kind: RuleBundleKind): void {
    ...
    for (const memberPath of memberFiles) {
        ...
        for (const [otherKind, count] of Object.entries(counts) as Array<[
            RuleBundleKind,
            number
        ]>) {
            if (otherKind === kind)
...
function main(): void {
        ...
        sources: [
            {
                ...
                surface: {
                    kind: "invoke",
                    methodName: "layoutEntry",
                    invokeKind: "static",
                    argCount: 1,
                    scope: { file: { mode: "equals", value: "layout_fixture.ets" } }
                },
                target: "arg0"
            }
        ]
        ...
        sinks: [
            {
                ...
                surface: {
                    ...
                    invokeKind: "static",
                    argCount: 1,
                    calleeScope: { file: { mode: "equals", value: "layout_fixture.ets" } }
                }
            }
        ]
    }));
    ...
    assert(loaded.appliedRuleSources.join(" -> ") === "kernel", "pack layout loading should keep kernel-only order by default");
