...
function expectedCompletionClass(spec: ExecutionHandoffCompareCase): CompletionClass {
...
function buildExpectedSignature(spec: ExecutionHandoffCompareCase): ExpectedSemanticSignature {
    return {
...
function normalizeCompletionClass(item: ExecutionHandoffContractSnapshotItem): CompletionClass {
    if (item.kernel.domain !== "deferred") {
        return "none";
    }
...
function mapObservedSignature(item: ExecutionHandoffContractSnapshotItem): ObservedSemanticSignature {
    return {
        ...
        carrierSemanticClass: item.kernel.domain === "deferred" ? "handoff" : "control",
...
async function runCaseAudit(
    ...
    if (!spec.factors.deferred) {
        const controlCandidate = observed.find(item => item.activationToken === "call(c)" && item.deferred === false);
        return {
            ...
            expected,
            observed,
            matched: observed.length === 0 || !!controlCandidate,
            selected: controlCandidate,
            score: observed.length === 0 ? 1 : (controlCandidate ? semanticScore(expected, controlCandidate) : 0),
        };
    }
