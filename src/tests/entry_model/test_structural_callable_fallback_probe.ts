...
function main(): void {
    ...
    if (report.newDiscoveries.length > 0) {
        ...
        for (const d of report.newDiscoveries) {
            console.log(`  ${d.ownerName}.${d.methodName} callable@[${d.callableArgIndexes}] → ${d.withFallback.layer}`);
        }
    }
    ...
    if (report.recognizedWithFallback < report.recognizedWithoutFallback) {
        throw new Error("Structural fallback caused regression — fewer recognitions than without it.");
    }
