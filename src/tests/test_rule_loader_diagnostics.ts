...
function main(): void {
    ...
    try {
        loadRuleSet({
            defaultRulePath: "src/rules/default.rules.json",
            frameworkRulePath: "src/rules/framework.rules.json",
    ...
    fs.writeFileSync(path.join(ruleDir, "default.rules.json"), emptyRuleSet, "utf-8");
    fs.writeFileSync(path.join(ruleDir, "framework.rules.json"), emptyRuleSet, "utf-8");
    fs.writeFileSync(path.join(ruleDir, "project.rules.json"), emptyRuleSet, "utf-8");
    fs.writeFileSync(path.join(ruleDir, "llm_candidate.rules.json"), emptyRuleSet, "utf-8");
    ...
    const warnedRules = loadRuleSet({
        ...
        defaultRulePath: path.join(ruleDir, "default.rules.json"),
        frameworkRulePath: path.join(ruleDir, "framework.rules.json"),
        projectRulePath: path.join(ruleDir, "project.rules.json"),
        llmCandidateRulePath: path.join(ruleDir, "llm_candidate.rules.json"),
    });
    assert(
