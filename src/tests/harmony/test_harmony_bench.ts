...
function readManifest(manifestPath: string): HarmonyBenchManifest {
    ...
    for (const [idx, categoryRaw] of root.categories.entries()) {
        ...
        const rules: HarmonyBenchRulePaths = {
            kernelRule: String(rulesObj.kernelRule || "").trim(),
            ruleCatalog: String(rulesObj.ruleCatalog || "").trim(),
            project: String(rulesObj.project || "").trim(),
        };
        if (!rules.kernelRule || !rules.ruleCatalog || !rules.project) {
            throw new Error(`Invalid categories[${idx}].rules: kernelRule/ruleCatalog/project are required`);
        }
...
function ensureRequiredPaths(manifestPath: string, manifest: HarmonyBenchManifest): void {
    for (const category of manifest.categories) {
        ...
        const defaultAbs = path.resolve(process.cwd(), category.rules.kernelRule);
        const frameworkAbs = path.resolve(process.cwd(), category.rules.ruleCatalog);
...
function inExpectedFlowRange(caseInfo: HarmonyBenchCase, targetFlowCount: number): boolean {
    ...
    if (caseInfo.expected_flow_count_max !== undefined && targetFlowCount > caseInfo.expected_flow_count_max) {
        return false;
    }
    return true;
}
...
function renderMarkdownReport(report: BenchmarkReport): string {
    ...
    if (report.unexpected_hits.length > 0) {
        ...
        for (const hit of report.unexpected_hits) {
            lines.push(`- [${hit.categoryId}] ${hit.caseId} (${hit.file}) flowCount=${hit.flowCount} target=${hit.targetFlowCount} spillover=${hit.spilloverFlowCount} expectedFlow=${hit.expectedFlow}`);
            for (const sample of hit.sinkSamples) {
                lines.push(`  - ${sample}`);
            }
            const sinkRuleIds = Object.keys(hit.sinkRuleHits || {}).sort();
            if (sinkRuleIds.length > 0) {
                lines.push(`  - sinkRuleHits: ${sinkRuleIds.map(id => `${id}:${hit.sinkRuleHits[id]}`).join(", ")}`);
            }
        }
        lines.push("");
    }
    ...
    if (report.failures.length > 0) {
        ...
        for (const f of report.failures) {
            lines.push(`- [${f.categoryId}] ${f.caseId} expected=${f.expectedFlow ? "T" : "F"} detectedTarget=${f.detectedTarget} flowCount=${f.flowCount} target=${f.targetFlowCount} spillover=${f.spilloverFlowCount}`);
        }
        lines.push("");
    }
...
async function main(): Promise<void> {
    ...
    for (const category of manifest.categories) {
        const sourceDirAbs = path.resolve(process.cwd(), category.sourceDir);
        const kernelRulePath = path.resolve(process.cwd(), category.rules.kernelRule);
        const ruleCatalogPath = path.resolve(process.cwd(), category.rules.ruleCatalog);
        ...
        for (const caseInfo of category.cases) {
            ...
            if (!category.supported && result.scored && result.detectedAny) {
                unexpectedHits.push({
                    ...
                    targetFlowCount: result.targetFlowCount,
                    spilloverFlowCount: result.spilloverFlowCount,
                    flowCount: result.flowCount,
                    sinkPattern: result.expectedSinkPattern,
                    sinkSamples: result.sinkSamples,
                    sinkRuleHits: result.sinkRuleHits,
                });
