...
function makeCaseSandbox(sourceDir: string, caseName: string): string {
    const sandboxRoot = fs.mkdtempSync(path.join(path.resolve("tmp"), "arkmain_stateful_container_"));
    fs.copyFileSync(path.join(sourceDir, `${caseName}.ets`), path.join(sandboxRoot, `${caseName}.ets`));
    return sandboxRoot;
}

async function main(): Promise<void> {
    const sourceDir = path.resolve("tests/demo/arkmain_stateful_container_precision");
    ...
    console.log("====== ArkMain Stateful Container Precision ======");
