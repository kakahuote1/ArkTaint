...
async function main(): Promise<void> {
    ...
    writeText(path.join(moduleRoot, "branching.rules.json"), stringifyRuleAssetFixture({
        id: "asset.rule.fixture.branching",
        sources: [
            {
    ...
    writeText(path.join(moduleProjectDir, "branching_bridge.ts"), [
    ...
    const normalizedPaths = materialized[0].paths
        ...
        .sort();
    assert(normalizedPaths[0] !== normalizedPaths[1], `expected two distinct materialized paths, got ${JSON.stringify(normalizedPaths)}`);
