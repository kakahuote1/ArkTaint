...
async function main(): Promise<void> {
    ...
    writeText(path.join(moduleRoot, "path_guard_literal_conflict.rules.json"), stringifyRuleAssetFixture({
    ...
    writeText(path.join(moduleProjectDir, "path_guard_literal_conflict_bridge.ts"), [
        ...
        "  setup(ctx) {",
        "    const storeCalls = ctx.scan.invokes({ methodName: \"Store\" }).filter(call => call.args().length >= 2);",
        "    return {",
        "      onFact(event) {",
    ...
    const entry = (report.entries || []).find(item => Array.isArray(item.postsolveResults) && item.postsolveResults.length > 0);
    ...
    assert(result.evidenceSummary.evidenceKinds.includes("path_guard"), `expected path_guard evidence, got ${JSON.stringify(result.evidenceSummary.evidenceKinds)}`);
    assert(result.paths.length > 0, "expected at least one materialized path");
    assert(result.paths.every(item => item.status !== "incomplete" && !item.truncated), `expected complete paths, got ${JSON.stringify(result.paths.map(item => ({ status: item.status, truncated: item.truncated })))}`);
    assert(result.paths.every(item => item.judgement.kind === "Refuted-Strong"), `expected all paths refuted, got ${JSON.stringify(result.paths.map(item => item.judgement.kind))}`);
    assert(result.paths.some(item => item.evidence.some(evidence => evidence.kind === "path_guard")), "expected path_guard evidence on at least one path");
    ...
    assert((materialized.evidenceKinds || []).includes("path_guard"), `expected materialized path_guard evidence, got ${JSON.stringify(materialized.evidenceKinds)}`);
