...
import { getAnalyzeDiagnosticsJsonPath, getAnalyzeDiagnosticsTextPath, getAnalyzeRunJsonPath, getAnalyzeSummaryJsonPath, } from "../helpers/AnalyzeCliRunner";
...
async function main(): Promise<void> {
    ...
    const privateSupportImport = path.relative(privateModuleDir, path.resolve("src/core/orchestration/modules/ModuleRuntime")).replace(/\\/g, "/");
    writeText(path.join(healthyModuleDir, "healthy.ts"), [
    ...
    writeText(brokenRulePath, stringifyRuleAssetFixture({
    ...
    assert((brokenDiagnosticsJson.items || []).some(item => item.category === "Module" && item.code === "MODULE_SETUP_THROW"), "broken module diagnostics.json should expose normalized module diagnostic items");
    ...
    assert((brokenSummary.summary?.diagnosticItems || []).some(item => item.category === "Module" && item.code === "MODULE_SETUP_THROW"), "broken module summary.json should expose normalized module diagnostic items");
    assert(brokenOutput.includes("with_flows=1") || brokenOutput.includes("total_flows=1"), "broken module analyze should still continue and produce baseline flows");
    ...
    assert((privateDiagnosticsJson.items || []).some(item => item.category === "Module" && item.code === "MODULE_PROJECT_PRIVATE_IMPORT"), "private-import diagnostics.json should expose normalized module diagnostic items");
