...
async function main(): Promise<void> {
    ...
    writeText(path.join(moduleRoot, "field.rules.json"), stringifyRuleAssetFixture({
    ...
    writeText(path.join(moduleProjectDir, "field_bridge.ts"), [
    ...
    const allPaths = materializedEntries.flatMap(entry => (entry.materializedTaintFlows || []).flatMap(item => item.paths || []));
    assert(allPaths.length > 0, "expected at least one materialized witness path");
    assert(allPaths.every(path => Array.isArray(path.factIds) && path.factIds.length >= 2), "expected every materialized witness path to contain at least 2 facts");
