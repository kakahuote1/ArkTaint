...
async function main(): Promise<void> {
    ...
    writeText(rulePath, stringifyRuleAssetFixture({
        id: "asset.rule.fixture.suppressed_flows",
        sources: [
            {
    ...
    assert(suppressed[0].evidenceSummary.evidenceKinds.includes("type_narrowing_guard"), `expected evidenceKinds to include type_narrowing_guard, got ${JSON.stringify(suppressed[0].evidenceSummary.evidenceKinds)}`);
    assert(suppressed[0].paths.length > 0, "expected suppressed flow to retain path details");
    assert(suppressed[0].paths.every(pathItem => pathItem.judgement.kind === "Refuted-Strong"), `expected all suppressed path judgements to be Refuted-Strong, got ${JSON.stringify(suppressed[0].paths.map(item => item.judgement.kind))}`);
    assert(suppressed[0].paths.some(pathItem => pathItem.evidence.some(evidence => evidence.kind === "type_narrowing_guard")), "expected suppressed path evidence to include type_narrowing_guard");
    assert((entry.materializedTaintFlows || []).length > 0, "expected materializedTaintFlows to retain refuted path judgement details");
