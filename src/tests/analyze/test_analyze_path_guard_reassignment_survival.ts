...
async function main(): Promise<void> {
    ...
    writeText(path.join(moduleRoot, "path_guard_reassignment_survival.rules.json"), stringifyRuleAssetFixture({
        id: "asset.rule.fixture.path_guard_reassignment_survival",
        sources: [
            {
    ...
    writeText(path.join(moduleProjectDir, "path_guard_reassignment_survival_bridge.ts"), [
    ...
    assert(result.judgement.kind !== "Refuted-Strong", `expected reassignment to prevent strong refutation, got ${result.judgement.kind}`);
    assert(!result.evidenceSummary.evidenceKinds.includes("path_guard"), `expected no path_guard evidence after reassignment, got ${JSON.stringify(result.evidenceSummary.evidenceKinds)}`);
    assert(result.paths.every(pathItem => !pathItem.evidence.some(evidence => evidence.kind === "path_guard")), "expected no path-level path_guard evidence after reassignment");
