...
async function main(): Promise<void> {
    ...
    writeText(rulePath, stringifyRuleAssetFixture({
        ...
        sinks: [{
                ...
                surface: { kind: "invoke", methodName: "Sink" },
                target: "arg0"
            }],
        sanitizers: [{
                id: "sanitizer.fixture.escape.result",
                surface: { kind: "invoke", methodName: "Escape" },
                target: "result"
            }],
        transfers: [{
    ...
    assert((entry.materializedTaintFlows || []).some(item => item.sinkFactId === sanitized!.flow.sinkFactId && item.judgement === "Refuted-Strong"), "expected materializedTaintFlows to include refuted flow judgement for report alignment");
