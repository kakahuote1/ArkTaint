...
async function main(): Promise<void> {
    const root = resolveTestRunDir("diagnostics", "analyze_keyed_route_callback_mismatch_suppressed");
    const repoRoot = resolveTestRunPath("diagnostics", "analyze_keyed_route_callback_mismatch_suppressed", "fixtures", "repo");
    const moduleRoot = resolveTestRunPath("diagnostics", "analyze_keyed_route_callback_mismatch_suppressed", "fixtures", "module_root");
    const baselineOutput = resolveTestRunPath("diagnostics", "analyze_keyed_route_callback_mismatch_suppressed", "runs", "baseline");
    const moduleOutput = resolveTestRunPath("diagnostics", "analyze_keyed_route_callback_mismatch_suppressed", "runs", "module");
    fs.rmSync(root, { recursive: true, force: true });

    const repoSourceDir = path.join(repoRoot, "src", "main", "ets");
    const moduleProjectDir = path.join(moduleRoot, "project", "keyed_route_callback_mismatch", "modules");
    ...
    writeText(
        ...
        [
            ...
            "    RegisterRoute(\"SafeDetail\", (param: string) => {",
            "      Sink(param);",
            "    });",
            "    PushRoute(\"OtherPage\", taint_src);",
            "    TriggerRoute(\"SafeDetail\");",
            "  }",
            "}",
            "",
        ].join("\n"),
    ...
    writeText(
        ...
        stringifyRuleAssetFixture({
            id: "asset.rule.fixture.keyed_route_callback_mismatch",
            sources: [
                {
                    id: "source.fixture.keyed_route_callback_mismatch",
            ...
            sinks: [
                {
                    id: "sink.fixture.keyed_route_callback_mismatch",
                    match: {
    ...
    runAnalyzeCli([
        ...
        "--enable-model", "keyed_route_callback_mismatch:modules",
    ...
    assert(withModule.summary.totalFlows === 0, `module surviving flows should be zero, got ${withModule.summary.totalFlows}`);
    ...
    const refuted = entry!.postsolveResults!.find(item =>
        item.judgement.kind === "Refuted-Strong"
        && item.evidenceSummary.evidenceKinds.includes("keyed_route_callback_mismatch"),
    );
    assert(refuted, "expected one Refuted-Strong keyed_route_callback_mismatch result");
    assert(
        refuted!.paths.every(pathItem => pathItem.judgement.kind === "Refuted-Strong"),
        `expected all path judgements to be Refuted-Strong, got ${JSON.stringify(refuted!.paths.map(item => item.judgement.kind))}`,
    );
    assert(
        refuted!.paths.some(pathItem => pathItem.evidence.some(evidence => evidence.kind === "keyed_route_callback_mismatch")),
