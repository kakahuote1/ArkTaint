...
async function main(): Promise<void> {
    ...
    const moduleProjectDir = path.join(moduleRoot, "project", "channel_demo", "modules");
    ...
    writeText(path.join(moduleRoot, "channel.rules.json"), stringifyRuleAssetFixture({
        id: "asset.rule.fixture.channel",
        sources: [
            {
    ...
    writeText(path.join(moduleProjectDir, "channel_bridge.ts"), [
        ...
        "    for (const call of ctx.scan.invokes({ methodName: \"registerChannel\" })) {",
        ...
        "      });",
        "    }",
        "    for (const call of ctx.scan.invokes({ methodName: \"dispatchChannel\" })) {",
        "      if (call.args().length < 2) continue;",
        "      const key = ctx.analysis.stringCandidates(call.arg(0))[0];",
        "      if (!key) continue;",
    ...
    runAnalyzeCli([
        ...
        "--enable-model", "channel_demo:modules",
