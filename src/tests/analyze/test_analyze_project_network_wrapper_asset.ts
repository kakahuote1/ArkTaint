...
async function main(): Promise<void> {
    ...
    writeText(path.join(etsRoot, "services", "NetworkManagerV2.ets"), [
    ...
    writeText(path.join(etsRoot, "services", "search", "SearchService.ets"), [
        ...
        sources: [
            {
                ...
                surface: { kind: "invoke", methodName: "Secret" },
                target: "result"
            }
        ],
        sinks: [],
    ...
    assert(hasSink(summary, "project.clearchat.NetworkManagerV2.sendStreamChatRequestV2.messages"), "tainted stream messages should hit the project wrapper body sink");
    assert(hasSink(summary, "project.clearchat.NetworkManagerV2.sendChatRequest.apiKey"), "tainted API key should hit the project wrapper authorization sink");
    assert(hasSink(summary, "project.clearchat.SearchService.makeRequest.headers"), "tainted headers should hit the project search wrapper header sink");
    assert(hasSink(summary, "project.clearchat.SearchService.makeRequest.body"), "tainted body should hit the project search wrapper body sink");
    assert(!hasSink(summary, "project.clearchat.NetworkManagerV2.sendStreamChatRequestV2.tools"), "taint in config.ignored must not satisfy the config.tools endpoint sink");
    assert(!hasSink(summary, "project.clearchat.SearchService.makeRequest.url"), "taint in HTTP method argument must not be treated as the URL endpoint");
