...
async function main(): Promise<void> {
    ...
    assert(
        (entry.transferNoHitReasons || []).some(reason => reason.startsWith("propagation_budget_exceeded")),
        `expected propagation budget reason, got ${(entry.transferNoHitReasons || []).join(",")}`,
    );
    assert(
        (entry.detectProfile?.detectCallCount || 0) > 0,
        "sink detection should run even after worklist truncation",
    );
    assert(
        (entry.stageProfile?.buildPagProfile?.totalMs || 0) > 0,
        "expected buildPAG subphase profile to be recorded",
    );
    assert(
        (entry.stageProfile?.sourceRulePropagationProfile?.totalMs || 0) > 0,
        "expected source-rule propagation subphase profile to be recorded",
    );
    assert(summary.summary.totalFlows > 0, `expected partial flow evidence, got ${summary.summary.totalFlows}`);
