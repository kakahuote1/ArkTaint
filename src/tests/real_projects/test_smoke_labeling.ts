...
function inventoryFlowCount(entry: Pick<SmokeEntry, "flowCount" | "flowRuleTraces" | "sinkRuleHits">): number {
...
function classifyCandidate(entry: SmokeEntry): CandidateType {
    ...
    if (inventoryFlowCount(entry) > 0) {
        return "flow_detected";
    }
    if (entry.status === "no_seed") return "no_seed";
    if (entry.seedCount > 0 && inventoryFlowCount(entry) === 0) {
        return "seed_no_flow";
    }
    return "other";
}
...
function buildPriority(entry: SmokeEntry, candidateType: CandidateType, projectPriority: string): number {
    ...
    score += inventoryFlowCount(entry) * 30;
    ...
    for (const project of report.projects || []) {
        ...
        for (const entry of project.entries || []) {
            ...
            items.push({
                ...
                flowCount: inventoryFlowCount(entry),
                sinkRuleHits: entry.sinkRuleHits || {},
                sinkEndpointHits: entry.sinkEndpointHits || {},
...
function sortCandidates(items: LabelItem[]): LabelItem[] {
    return [...items].sort((a, b) => {
        ...
        const bInventory = sumCounts(b.sinkRuleHits) > 0 ? sumCounts(b.sinkRuleHits) : b.flowCount;
        const aInventory = sumCounts(a.sinkRuleHits) > 0 ? sumCounts(a.sinkRuleHits) : a.flowCount;
...
function renderSummaryMarkdown(labelFile: SmokeLabelFile, fileName: string): string {
    ...
    lines.push("# Smoke Label Summary");
    lines.push("");
    ...
    for (const item of unresolved) {
        const topSinkRules = Object.entries(item.sinkRuleHits || {})
            .sort((a, b) => b[1] - a[1])
            .slice(0, 2)
            .map(([id, count]) => `${id}:${count}`)
            .join(",");
        lines.push(`- [${item.projectId}] ${item.entryName} @ ${item.entryPathHint || "N/A"} | type=${item.candidateType} | status=${item.status} | flows=${item.flowCount} | seeds=${item.seedCount} | priority=${item.priorityScore}${topSinkRules ? ` | sinkRuleHits=${topSinkRules}` : ""}`);
    }
    lines.push("");
