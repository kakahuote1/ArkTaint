...
async function main(): Promise<void> {
    ...
    const packImports = collectCrossRootImports(PACK_ROOT);
    ...
    const unexpectedPackImports = packImports.filter(edge => !isAllowedPackImport(edge));
    ...
    assert(
        unexpectedPackImports.length === 0,
        `Layer dependency gate found unexpected pack -> core imports:\n${unexpectedPackImports.map(edge =>
            `  PACK ${edge.fromFile} -> ${edge.toLayer} ${edge.toFile}`,
        ).join("\n")}`,
    );
    assert(
    ...
    console.log(`pack_core_imports=${packImports.length}`);
