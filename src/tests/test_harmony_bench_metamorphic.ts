...
function matchesSinkPattern(stmtText: string, pattern: string): boolean {
    ...
    if (p.startsWith("/") && p.endsWith("/") && p.length > 2) {
        return new RegExp(p.slice(1, -1)).test(stmtText);
    }
    return stmtText.includes(p);
}

function classify(expectedFlow: boolean, detectedByPattern: boolean, detectedAny: boolean): "TP" | "FP" | "TN" | "FN" {
...
async function runCase(
    ...
    try {
        ...
        const matched = flows.filter((f: TaintFlow) => matchesSinkPattern(f.sink.toString(), caseInfo.expected_sink_pattern));
        const detectedByPattern = matched.length > 0;
        const detectedAny = flows.length > 0;
        const classification = classify(caseInfo.expected_flow, detectedByPattern, detectedAny);
        const pass = caseInfo.expected_flow ? detectedByPattern : !detectedAny;
...
function renderSummaryMarkdown(eq: EqReport, challenge: ChallengeReport, options: CliOptions): string {
    ...
    lines.push("");
    lines.push("- Group A: strict equivalence mutations. Baseline and mutation must match under `expected_flow + expected_sink_pattern`.");
...
function renderGapBacklog(eq: EqReport, challenge: ChallengeReport): string {
    ...
    for (const item of challenge.items) {
        ...
        gapItems.push({
            ...
            evidence: `flows=${item.flowCount}, matched=${item.matchedFlowCount}, sink=${item.sinkSamples[0] || "N/A"}, hits=${formatTopRuleHits(item.ruleHits)}`,
        });
    }
...
async function main(): Promise<void> {
    ...
    for (const mutation of dataset.mutations) {
        ...
        const rulesKey = [
            path.resolve(mutation.rulePaths.default),
            path.resolve(mutation.rulePaths.framework),
            path.resolve(mutation.rulePaths.project),
        ].join("|");
        let loadedRules = rulesCache.get(rulesKey);
        if (!loadedRules) {
            loadedRules = loadRuleSet({
                defaultRulePath: path.resolve(mutation.rulePaths.default),
                frameworkRulePath: path.resolve(mutation.rulePaths.framework),
        ...
        if (mutation.group === "A") {
            const consistent = baseline.detectedByPattern === mutated.detectedByPattern
            ...
            aItems.push({
                ...
                baselineDetected: baseline.detectedByPattern,
                mutatedDetected: mutated.detectedByPattern,
            ...
            bItems.push({
                ...
                detectedByPattern: mutated.detectedByPattern,
                flowCount: mutated.flowCount,
                matchedFlowCount: mutated.matchedFlowCount,
    ...
    const unrelatedSinkHits = bItems.reduce((acc, x) => acc + Math.max(0, x.flowCount - x.matchedFlowCount), 0);
