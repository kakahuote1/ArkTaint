...
function parseArgs(argv: string[]): CliOptions {
    let sourceDir = "tests/demo/rule_precision_sanitizer";
    let defaultRulePath = "tests/rules/minimal.rules.json";
    let projectRulePath = "tests/rules/sanitizer_guard.rules.json";
    let k = 1;
...
async function main(): Promise<void> {
    ...
    const sanitizerRules: SanitizerRule[] = loaded.ruleSet.sanitizers || [];
    console.log(`source_rules_loaded=${sourceRules.length}`);
    console.log(`sink_rules_loaded=${sinkRules.length}`);
    console.log(`sanitizer_rules_loaded=${sanitizerRules.length}`);
    ...
    console.log("====== Sanitizer Guard Test ======");
