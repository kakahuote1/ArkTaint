...
export interface BuildRouterModelArgs {
    scene: Scene;
    pag: Pag;
...
export function buildRouterModel(args: BuildRouterModelArgs): RouterModel {
    ...
    const instInitPayloadNodeIdsByClassType = new Map<string, Set<number>>();
    const instInitRouteLiteralsByClassAndField = new Map<string, string[]>();
        ...
        for (const stmt of cfg.getStmts()) {
            ...
            if (pushRouterKey) {
                ...
                const payload = collectPushPayload(
                    ...
                    invokeExpr,
                    args.pag,
                    instInitPayloadNodeIdsByClassType,
                    instInitRouteLiteralsByClassAndField,
                    pushRouterKey,
                    invokeMethodName
                );
            ...
            if (
                ...
                const navRouteKeys = collectNavDestinationRouteKeys(method, invokeExpr);
...
function collectPushPayload(
    ...
    invokeExpr: any,
    pag: Pag,
    instInitPayloadNodeIdsByClassType: Map<string, Set<number>>,
    instInitRouteLiteralsByClassAndField: Map<string, string[]>,
    routerKey: string,
    ...
    for (const arg of argsList) {
        ...
        for (const stmt of stmts) {
            ...
            if (routeFieldName && fieldName === routeFieldName) {
                const literal = tryParseStringLiteral(right);
                if (literal) {
                    routeLiteralKeys.add(`${routerKey}::${routeFieldName}=${literal}`);
                    routeLiteralKeys.add(`${routeFieldName}=${literal}`);
                    routeLiteralKeys.add(`route=${literal}`);
                }
            }
        }

        const classType = String(arg.getType?.()?.toString?.() || "").trim();
        if (!classType) continue;
        let payloadNodeIds = instInitPayloadNodeIdsByClassType.get(classType);
        if (!payloadNodeIds) {
            payloadNodeIds = collectInstInitPayloadNodeIds(scene, pag, classType);
            instInitPayloadNodeIdsByClassType.set(classType, payloadNodeIds);
        }
        ...
        if (routeFieldName) {
            const cacheKey = `${classType}#${routeFieldName}`;
            let literals = instInitRouteLiteralsByClassAndField.get(cacheKey);
            if (!literals) {
                literals = collectInstInitRouteLiterals(scene, classType, routeFieldName);
                instInitRouteLiteralsByClassAndField.set(cacheKey, literals);
            }
...
function collectNavDestinationRouteKeys(method: any, invokeExpr: any): string[] {
    ...
    for (const arg of invokeArgs) {
        const literal = tryParseStringLiteral(arg);
        if (literal) {
            addLiteral(literal);
            continue;
        }
        ...
        for (const stmt of stmts) {
            ...
            const fieldLiteral = tryParseStringLiteral(stmt.getRightOp());
            if (fieldLiteral) addLiteral(fieldLiteral);
        }
    }
...
function collectInstInitPayloadNodeIds(scene: Scene, pag: Pag, classType: string): Set<number> {
    const out = new Set<number>();
    ...
    for (const method of scene.getMethods()) {
        ...
        for (const stmt of cfg.getStmts()) {
            ...
            if (!rightNodes || rightNodes.size === 0) continue;
...
function resolveRouteFieldNameForPushMethod(methodName: string): string | undefined {
    ...
    if (methodName === "pushPath" || methodName === "pushPathByName") return "name";
    if (methodName === "pushUrl" || methodName === "replaceUrl") return "url";
    return undefined;
}

function collectInstInitRouteLiterals(scene: Scene, classType: string, fieldName: string): string[] {
    const out = new Set<string>();
    const escapedClassType = escapeForRegex(classType);
    const instInitPattern = new RegExp(`${escapedClassType}\\.\\%instInit\\(`);
    ...
    for (const method of scene.getMethods()) {
        ...
        for (const stmt of cfg.getStmts()) {
            ...
            if (currentField !== fieldName) continue;
            const literal = tryParseStringLiteral(stmt.getRightOp());
