...
import { resolveMethodsFromCallable } from "../engine/CalleeResolver";
import { collectNodeIdsFromValue, collectObjectNodeIdsFromValue, resolveHarmonyMethods } from "./HarmonyModelingUtils";

export interface WorkerTaskPoolModel {
...
export interface BuildWorkerTaskPoolModelArgs {
    scene: Scene;
    pag: Pag;
...
function collectCallbackParamNodeIds(
    ...
    for (const callbackMethod of callbackMethods) {
        const paramLocal = resolveParameterLocal(callbackMethod, paramIndex);
        if (!(paramLocal instanceof Local)) continue;
        const nodes = pag.getNodesByValue(paramLocal);
...
function resolveParameterLocal(method: any, paramIndex: number): Local | undefined {
    const cfg = method.getCfg?.();
    ...
    for (const stmt of cfg.getStmts()) {
        ...
        if (right.getIndex() !== paramIndex) continue;
