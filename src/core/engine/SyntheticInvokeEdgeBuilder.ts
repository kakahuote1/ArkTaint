...
function summarizeConstructorCapturedLocalToFields(
    scene: Scene,
    method: any,
...
function injectAsyncCallbackCaptureEdges(
    ...
    const asyncNames = new Set(["setTimeout", "setInterval", "queueMicrotask"]);
    ...
    const callbackMethods = resolveAsyncCallbackMethods(scene, cg, caller, callbackArg, callsiteLine, context);
    if (callbackMethods.length === 0) return 0;

    const body = caller.getBody?.();
    if (!body) return 0;
    const callerLocals = body.getLocals?.();
    if (!callerLocals) return 0;
    ...
    for (const cbMethod of callbackMethods) {
        ...
        if (paramStmts.length > 0) {
            for (const paramStmt of paramStmts) {
                ...
                const capturedName = paramLocal.getName();
                const callerLocal = callerLocals.get(capturedName);
                if (!(callerLocal instanceof Local)) continue;

                const srcNodes = pag.getNodesByValue(callerLocal);
                let dstNodes = pag.getNodesByValue(paramLocal);
        ...
        for (const mapping of capturedLocalMappings) {
            const callerLocal = callerLocals.get(mapping.callerLocalName);
            if (!(callerLocal instanceof Local)) continue;

            const srcNodes = pag.getNodesByValue(callerLocal);
            let dstNodes = pag.getNodesByValue(mapping.callbackValue);
            if ((!dstNodes || dstNodes.size === 0) && mapping.anchorStmt) {
                pag.addPagNode(0, mapping.callbackValue, mapping.anchorStmt);
                dstNodes = pag.getNodesByValue(mapping.callbackValue);
            }
            if (!srcNodes || !dstNodes) continue;
...
function resolveAsyncCallbackMethods(
    scene: Scene,
    cg: CallGraph,
    callerMethod: any,
    callbackArg: any,
    ...
    if (callbackParamIndex !== undefined && callbackParamIndex >= 0) {
        ...
        return resolveAsyncCallbackMethodsFromOneHopCallers(scene, cg, callerMethod, callbackParamIndex, context);
    }
...
function resolveAsyncCallbackMethodsFromOneHopCallers(
    scene: Scene,
    cg: CallGraph,
    callerMethod: any,
    ...
    return resolveOneHopCallableMethodsFromParamIndex(scene, cg, callerMethod, callbackParamIndex, 8, context);
}

function resolveOneHopCallableMethodsFromParamIndex(
    scene: Scene,
    cg: CallGraph,
    calleeMethod: any,
    ...
    for (const cs of incomingCallSites) {
        ...
        for (const pair of argToParamPairs) {
            ...
            // Hard constraint for 7.8.3 one-hop summary: must have callable evidence.
            if (!isCallableValue(pair.arg)) continue;

            const methods = resolveMethodsFromCallable(scene, pair.arg, { maxCandidates });
...
function resolveReflectDispatchOneHopFallbackCallees(
    ...
    for (const candidate of candidateValues) {
        ...
        const methods = resolveOneHopCallableMethodsFromParamIndex(scene, cg, callerMethod, paramIndex, 8, context);
...
function resolveMethodsFromAnonymousObjectCarrier(scene: Scene, objectValue: any): any[] {
    ...
    if (!classSig) return [];
    if (!classSig.includes(": %AC")) return [];
