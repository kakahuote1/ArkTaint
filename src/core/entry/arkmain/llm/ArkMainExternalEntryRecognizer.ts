...
export async function recognizeExternalArkMainEntries(
    ...
    if (!options.modelInvoker) {
        const fallback = fallbackHeuristicRecognitions(selectedCandidates, minConfidence);
        persistRecognitionsToCache(cache, fallback, selectedCandidates, options.model);
    ...
    for (const batch of chunkCandidates(uncachedCandidates, batchSize)) {
        ...
        try {
            ...
            const fallback = fallbackHeuristicRecognitions(batch, minConfidence);
            results.push(...fallback);
            persistRecognitionsToCache(cache, fallback, batch, options.model);
            continue;
        }
...
function fallbackHeuristicRecognitions(
    candidates: ArkMainExternalEntryCandidate[],
    ...
    for (const candidate of candidates) {
        const ownerScore = candidate.ownerSignals.length * 0.16;
        const overrideScore = candidate.overrideSignals.length * 0.14;
        const frameworkScore = candidate.frameworkSignals.length * 0.08;
        const rawScore = ownerScore + overrideScore + frameworkScore;
        const confidence = Math.max(0, Math.min(0.98, rawScore));

        if (confidence < minConfidence) {
            continue;
        }

        const phase = inferPhaseFromCandidate(candidate);
        const kind = inferKindFromCandidate(candidate);
        if (!phase || !kind) {
            continue;
        }
        ...
        out.push({
            ...
            evidenceTags: buildFallbackEvidenceTags(candidate),
        });
    }

    return dedupeAndSortRecognitions(out);
}
...
function buildFallbackReason(candidate: ArkMainExternalEntryCandidate): string {
    const pieces: string[] = [];

    if (candidate.ownerSignals.length) {
        pieces.push(`owner=${candidate.ownerSignals.slice(0, 2).join("|")}`);
    }
    if (candidate.overrideSignals.length) {
        pieces.push(`override=${candidate.overrideSignals.slice(0, 2).join("|")}`);
    }
    if (candidate.frameworkSignals.length) {
        pieces.push(`framework=${candidate.frameworkSignals.slice(0, 3).join("|")}`);
    }

    return `[heuristic external entry] ${pieces.join("; ") || "strong framework-like signals"}`;
}

function buildFallbackEvidenceTags(candidate: ArkMainExternalEntryCandidate): string[] {
    return [
        ...candidate.ownerSignals.slice(0, 3),
        ...candidate.overrideSignals.slice(0, 3),
        ...candidate.frameworkSignals.slice(0, 4),
    ].filter(Boolean);
}
